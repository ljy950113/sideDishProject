...
public class Manager implements Food{
	...
	String[] soupName = {"갈비탕", "육개장", "된장찌개", "김치찌개"}; // 탕종류 배열 0열에 육개장 1열에 김치찌개 이런식
	int[] soupPrice = new int[4];
	int[] soupAmount = new int[4]; // 0갈비탕 1육개장 2된장찌개 3김치찌개 4부대찌개
	String[] sideName = {"무말랭이", "메추리알", "콩자반", "멸치볶음"};// 반찬류 배열 0열에 배추김치, 1열에 갓김치
	int[] sidePrice  = new int[4];
	int[] sideAmount = new int[4];
	int password = 1234;
	boolean flag = true;
	...
	public void entering() {
		...
		while(run) {
			...
			switch(select) {
			case 1: 
				System.out.println("1. 갈비탕 | 2. 육개장 | 3. 된장찌개 | 4. 김치찌개");
				System.out.println("선택>");
				...
				break;
			case 2:
				System.out.println("1. 무말랭이 | 2. 메추리알 | 3. 콩자반 | 4. 멸치볶음");
				System.out.println("선택>");
					selectNo = sc.nextInt();
				if(selectNo == 1) {
	...
	public void search() {
			...
			System.out.println("5.무말랭이 |" + sideAmount[0] +  ", " +"??" + "??");
			System.out.println("6.메추리알 |" + sideAmount[1] +  " ," +"??" + "??");
			System.out.println("7.콩자반 |" + sideAmount[2] +  " ," +"??" + "??");
			System.out.println("8 멸치볶음 |" + sideAmount[3] +  ", " +"??" + "??");
	}
	public void delete() {
